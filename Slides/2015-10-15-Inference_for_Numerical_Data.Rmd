...
$$\bar{x}_{male} = 626.9$$
$$\bar{x}_{female} = 597.7$$

$$(\bar{x}_{male} - \bar{x}_{female}) \pm z* \times SE_{\bar{x}_{male} - \bar{x}_{female}} = (626.9 - 595.7) \pm 1.96 \times 1.55$$
 
$$ (28.162, 34.238) $$ 

Male students score between 28.162 and 34.238 points higher than females on the math SAT.

## Null Hypothesis Testing

* $H_0$: $\mu_{male} = \mu_{female}$ or $H_0$: $\mu_{male} - \mu_{female} = 0$   
There is no difference in math SAT scores between males and females.
* $H_A$: $\mu_{male} \ne \mu_{female}$  or $H_0$: $\mu_{male} - \mu_{female} \ne 0$  
There is a difference in the average math SAT scores between males and females.

$$ Z = \frac{(\bar{x}_{male} - \bar{x}_{female}) - 0 }{SE_{\bar{x}_{male} - \bar{x}_{female} }} = \frac{31.2}{1.55} = 20.13 $$

```{r}
1 - pnorm(20.13) # Upper tail
```
...
<img src='images/William_Sealy_Gosset.jpg' align='right'>
...
<center><img src='images/tdistribution.png' width='50%'></center>
...
<center>
```{r comparetdistributions, width=6, height=3.5, echo=FALSE}
